...
public partial class ChangePassword : System.Web.UI.Page
{
    ...
    {
        string pass = OldPassword.Text.Trim();
        string newpass = NewPassword.Text.Trim();
        string renew = ReNewPassword.Text.Trim();

        if(pass == null || "".Equals(pass) || newpass == null || "".Equals(newpass)|| renew == null || "".Equals(renew)){
        ...
        else
        {
            if(newpass.Equals(renew)){
                try
                {
                    ...
                    if(q.Any()){
                        foreach(user_detail u in q){
                            ...
                            string newencpass = MainClass.getEncriptedString(newpass);
                            if(u.pass.Equals(encpass)){
                                u.pass = newencpass;
                            }
                            else
                            {
                        ...
                        OldPassword.Text = NewPassword.Text = ReNewPassword.Text = "";
                    }
                    else
                    {
                ...
                {
                    System.Console.Write("Exception In Password Changing");
                }
                
            }
            else
            {
                ErrorLabel.Text = "Both Passwords Are Not Equal";
            }
        }
        UpdatePanel1.Update();
    }
